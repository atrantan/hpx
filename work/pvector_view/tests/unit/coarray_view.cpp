...
int main()
{
    auto image_coarray =
        [](hpx::parallel::spmd_block block)
        {
            ...
            hpx::coarray<double,2,std::vector<double>> a
                = { block, "a", {N,N}, std::vector<double>(elt_size) };
            ...
            {
                hpx::coarray_view<double,2,std::vector<double>> view (
                    block, &a(i,j), &a(i+tile-1,j+tile-1), {tile,tile}, {N,N} );
            ...
            if(block.this_image() == 0)
            {
                ...
                for (std::size_t i = 0; i<N; i+=tile)
                {
                    ...
                    for (std::size_t ii = i, ii_end = i+tile; ii< ii_end; ii++)
                    {
                        ...
                        std::vector<double> value = a.get(ii,jj);
